...
/// bound to, plus a way to launch Windows' own "Update Driver" dialog for one — the first step
/// toward hiding a real XInput pad from a game without HIDHide (which can't touch XInput at all;
/// see the Bindings tab's design notes). This does NOT change any driver binding itself: rebinding
/// programmatically (which INF section to pick, across Windows versions) is the fragile,
/// hard-to-verify-without-hardware part, so for now this only detects and hands off to the OS's
/// own interactive wizard, which already knows how to do it safely and offers "Roll Back Driver"
